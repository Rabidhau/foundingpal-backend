...
const getActiveIdea = async (req, res) => {
  try {
    // Extracting search query & userId from request query params
    const { query, userId } = req.query;

    // Ensure userId is present, or return all active ideas (if allowed)
    ...
    conn.query(selectQuery, queryParams, (err, result) => {
      ...
      if (result.length === 0) {
        return res.status(404).json({ message: "No active ideas found" });
      }
